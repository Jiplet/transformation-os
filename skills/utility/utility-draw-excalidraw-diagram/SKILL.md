...
**Consistency:** If one panel uses bullet points (`•  `), every other multi-line description on the diagram uses bullets too. Don't mix bulleted lists with raw line-break paragraphs across cards. Same for numbered lists, dashes, indentation. Pick one style per diagram and stick to it.

**Professional tone:** Phrase objectives, descriptions, and gotchas in neutral business language. Don't name individuals when describing tasks ("Lift Rob's logic out of his head", "Sarah owns this in her brain"). Use "Document the existing logic", "Capture the current approach", "Existing rules are not yet documented" instead. Names are fine in attribution lines (e.g. "Open Questions for Rob") but not inside task phrasing.
...
**HARD RULE — never use the handwriting font (Virgil, fontFamily 1).** It looks unprofessional and is not acceptable for any Ventia output (exec, internal, or working). Default `fontFamily: 2` (Helvetica) for every text element. Use `fontFamily: 3` (Cascadia) only for code-style labels. Never `fontFamily: 1`.

Apply the same rule to `roughness`: use `0` (smooth) by default. The "hand-drawn" rough strokes pair with the handwriting font to create the same unprofessional look. Only deviate (`roughness: 1`) for an explicit, casual brainstorm context Jacob has confirmed.

| Role | fontSize | fontFamily |
|------|----------|------------|
| Diagram title | 28-32 | 2 (Helvetica) |
| Section header | 18-22 | 2 |
| Element label | 15-17 | 2 |
| Annotation | 13-14 | 2 |
| Small note | 11-12 | 2 |
| Code label | 13-15 | 3 (Cascadia) |
...
### Key values
- **fontFamily:** 2 = Helvetica (DEFAULT — always use), 3 = Cascadia (monospace, code labels only). **Never use 1 (Virgil / handwritten) — banned for professional output.**
- **roughness:** 0 = smooth (DEFAULT — always use). Never 1 or 2 unless Jacob has explicitly confirmed a casual / sketchy context.
- **fillStyle:** `"solid"` for clean diagrams (DEFAULT). Never `"hachure"` — pairs with the rough/handwritten look we are avoiding.
